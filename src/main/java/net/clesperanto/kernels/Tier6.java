...
public class Tier6 {
	...
	/**
	 * Removes holes in labelled objects samller than a given size (in pixels) from a label map.
	 * @param device ({@link DeviceJ}) - Device to perform the operation on.
	 * @param input ({@link ArrayJ}) - Label image to filter.
	 * @param output ({@link ArrayJ}) - Output label image fitlered. (default: None)
	 * @param max_size (float) - Biggest size holes allowed. (default: 100)
	 * @return {@link ArrayJ}
	 * @see https://clij.github.io/clij2-docs/reference_excludeLabelsOutsideSizeRange
	 * @throws NullPointerException if any of the device or input parameters are null.
	 */
    public static ArrayJ removeHoles(DeviceJ device, ArrayJ input, ArrayJ output, float max_size) {
        Objects.requireNonNull(device, "device cannot be null");
		Objects.requireNonNull(input, "input cannot be null");
        return new ArrayJ(net.clesperanto._internals.kernelj.Tier6.remove_holes(device.getRaw(), input.getRaw(), output == null ? null : output.getRaw(), max_size), device);
    }
